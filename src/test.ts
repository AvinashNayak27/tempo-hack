...
const USDC_TOKEN = '0x20c0000000000000000000000000000000000001';

// Wallet 1 - test account
const WALLET_1 = {
  address: '0x031891A61200FedDd622EbACC10734BC90093B2A' as const,
  privateKey: '0x2b9e3b8a095940cf3461e27bfb2bebb498df9a6381b76b9f9c48c9bbdc3c8192' as const,
};

const PRICE_PER_CALL = parseUnits('0.1', 6); // $0.1

async function main() {
  const providerWallet = process.env.PROVIDER_WALLET_ADDRESS;
  if (!providerWallet) {
    console.error('Set PROVIDER_WALLET_ADDRESS in env (or server .env) - use the API wallet address');
    process.exit(1);
  }

  console.log('Wallet 1:', WALLET_1.address);
  console.log('Provider wallet:', providerWallet);
  console.log('Price per call: $0.1 (100000 = 0.1 USDC)\n');
  ...
  console.log('\nAttestation: model=%s, total_tokens=%d', parsed.model, parsed.usage.total_tokens);

  // 4. Test /credits endpoint with signed attestation
  console.log('\n4. Calling /credits...');
  const creditsRes = await fetch(`${BASE_URL}/credits`);
  if (!creditsRes.ok) {
    console.error('Credits request failed:', creditsRes.status, await creditsRes.text());
    process.exit(1);
  }
  ...
  if (creditsData.attestation) {
    ...
    if (validCredits) {
      ...
      console.log('   Signed payload: total_credits=%s, total_usage=%s', creditsPayload.total_credits, creditsPayload.total_usage);
    } else {
